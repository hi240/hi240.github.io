...
public class BMICheck {
	...
	private void save() {
		...
		try{
			FileOutputStream fos = new FileOutputStream(file);          //input과 output이 언제 진행되는지헷갈린다.
			ObjectOutputStream oos = new ObjectOutputStream(fos);
			
			oos.writeObject(map); //map안에는 (1,new BMI)(2,new BMI)..등 keyValue형태로 들어가 있음.
	...
	private void load() {
		...
		try{
			...
			map = (HashMap)oos.readObject(); //객체에서 Object를 읽는다. 다운캐스팅한 oos오브젝트를 읽어서 해쉬맵에 저장시킨다.
