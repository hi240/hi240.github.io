...
public class Weight_Manager {
    ...
    public void inputWeight() throws IOException{
        ...
        if( !f.exists()){      //해당파일이 이전에 만들어진적이 없다면,
            ...
            if (!fd.exists()) fd.mkdir(); // 만약, fd 파일이 존재하지 않다면, C://WeightMgData/폴더 생성
            FileWriter fw = new FileWriter(f); //이 특수한 사람의 파일f를 생성한다.......  ???????????
            BufferedWriter bw = new BufferedWriter(fw); //buffer로 fw 읽어서 성능 높인다. .
            bw.write(this.str_weight+":");  //생성한 파일에 buffer통해 기록한다...
